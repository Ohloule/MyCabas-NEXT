...
export default function SearchResults({
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-16">
        <Loader taille={45} />
        <p className="mt-4 text-neu-600">Recherche en cours...</p>
      </div>
    );
  }
  ...
  if (!query && !categorySlug) {
    return (
      <div className="flex flex-col items-center justify-center py-16">
        ...
        <p className="mt-4 text-neu-600 text-center">
  ...
  return (
    <div>
      ...
      <div className="mb-6 text-sm text-neu-600">
        ...
        {resolvedQuery && resolvedQuery !== query.toUpperCase() && (
          <span className="block mt-1 text-xs text-neu-400">
            Recherche interprétée comme :{" "}
            <span className="font-medium text-neu-500">{resolvedQuery}</span>
          </span>
        )}
      </div>
