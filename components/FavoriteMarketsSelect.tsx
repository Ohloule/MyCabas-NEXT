...
export default function FavoriteMarketsSelect() {
  ...
  if (favorites.length === 0)
    return (
      <Link
        href="/markets"
        className="flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-prin-400/60 text-prin-200 hover:bg-prin-600/50 hover:text-blanc transition-all text-xs font-medium"
      >
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        className="flex items-center gap-2 cursor-pointer hover:opacity-80 transition-opacity text-prin-100"
      >
      ...
      {isOpen && (
        <div className="absolute right-0 mt-4 w-56 lg:w-max bg-prin-50 rounded-lg shadow-lg py-2 z-50 border border-neu-100">
          {favorites.map((f) => (
            <div
              key={f.id}
              className="flex items-center gap-2 px-4 py-2 hover:bg-prin-100 transition-colors"
            >
              <a
                ...
                className="shrink-0 text-prin-500 hover:text-prin-700 transition-colors"
