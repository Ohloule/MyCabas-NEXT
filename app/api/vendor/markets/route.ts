...
export async function POST(request: NextRequest) {
  try {
    ...
    const invalidDays = days.filter(
      (d: string) => !validDays.includes(d as Day),
    );
    ...
    const invalidSelections = days.filter(
      (d: string) => !marketDays.includes(d as Day),
    );
    ...
    return NextResponse.json(
      {
        error: "Erreur lors de l'inscription au marché",
