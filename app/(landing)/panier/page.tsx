  ...
  const handleUpdateQuantity = async (
    ...
    try {
      if (newQuantity <= min) {
        ...
        const validQty = parseFloat(
          (min + stepsAboveMin * step).toFixed(decimals),
        );
    ...
    return (
      <>
        ...
        <div className="align-center py-16 text-center">
          ...
          <Link href="/markets">
            <Button className="bg-prin-500 hover:bg-prin-600 gap-2">
  ...
  return (
    <>
      ...
      <div className="align-center py-8 pb-28 lg:pb-8">
        {/* Infos marché */}
        {cart.market && (
          <Card className="mb-6 border-prin-200 bg-prin-50 shadow-none">
            <CardContent className="flex items-center gap-4 px-4 py-3">
              <div className="flex items-center justify-center bg-prin-100 rounded-full p-2 shrink-0">
                <MapPin className="h-4 w-4 text-prin-600" />
              </div>
              ...
              {cart.marketDay && (
                <>
                  ...
                  <div className="flex flex-col items-center gap-1 shrink-0">
                    <Calendar className="h-4 w-4 text-prin-500" />
                    <Badge className="bg-prin-100 text-prin-800 border-prin-300 border text-xs font-semibold shadow-none">
        ...
        <div className="grid lg:grid-cols-3 gap-6 lg:gap-8">
                  ...
                  {items.map((item) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3 sm:gap-4   min-w-0 ">
                        {/* Image produit */}
                        <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-lg bg-neu-100 overflow-hidden shrink-0">
                          {item.product.imageUrl ? (
                            ...
                            <div className="w-full h-full flex items-center justify-center">
                              <ShoppingCart className="h-6 w-6 text-neu-300" />
                            </div>
                          )}
                        </div>
                        ...
                        {/* Infos produit */}
                        <div className="flex-1 min-w-0">
                          <h3 className="font-medium text-neu-900 truncate text-sm sm:text-base">
                            {item.product.name}
                          </h3>
            ...
            <Card className="sticky top-4">
              <CardContent className="p-4 sm:p-6 space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1 bg-neu-100 rounded-lg p-1">
                    <button
                      ...
                      className={`flex items-center gap-1 px-2 py-1 rounded-md text-xs font-medium transition-colors cursor-pointer ${
                        recapView === "vendor"
                          ? "bg-neu-50 text-prin-700 shadow-sm"
                ...
                <div className="space-y-3 text-sm">
                  {recapView === "vendor"
                    ? Object.values(itemsByVendor).map(({ vendor, items }) => {
                        ...
                        return (
                          <div key={vendor.id}>
                            <div className="flex items-center justify-between font-medium text-neu-700 mb-1">
                              <span className="flex items-center gap-1">
                                <Store className="h-3 w-3 text-prin-500 shrink-0" />
                                {vendor.stallName}
                              </span>
                              <span className="shrink-0 text-prin-600">
                                {subtotal.toFixed(2)} €
                              </span>
                            </div>
                            {items.map((item) => (
                    ...
                    : Object.values(itemsByCategory).map(({ name, items }) => {
                        ...
                        return (
                          <div key={name}>
                            <div className="flex items-center justify-between font-medium text-neu-700 mb-1">
                              <span className="flex items-center gap-1">
                                <Tag className="h-3 w-3 text-prin-500 shrink-0" />
                                {name}
                              </span>
                              <span className="shrink-0 text-prin-600">
                                {subtotal.toFixed(2)} €
                ...
                <div className="border-t pt-4">
                  <div className="flex justify-between font-semibold text-lg">
                    ...
                    <span className="text-prin-600">{total.toFixed(2)} €</span>
                  </div>
          ...
          <div className="fixed bottom-0 left-0 right-0 bg-neu-50 border-t shadow-lg p-4 lg:hidden z-50">
            <div className="flex items-center justify-between gap-4 max-w-2xl mx-auto">
              <div>
                <p className="text-xs text-neu-500">
                  {itemCount} {itemCount > 1 ? "produits" : "produit"}
                </p>
                <p className="font-semibold text-lg text-prin-600">
                  {total.toFixed(2)} €
                </p>
              </div>
