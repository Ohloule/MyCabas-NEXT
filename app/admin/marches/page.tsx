...
import { MapPin, CheckCircle, XCircle, Search, ChevronDown } from "lucide-react";
...
export default function AdminMarchesPage() {
  ...
  const buildUrl = useCallback(
    (currentSkip: number, currentSearch: string, currentFilter: MarketStatus | "ALL") => {
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">
            {total} marché{total !== 1 ? "s" : ""}{filter !== "ALL" ? ` ${STATUS_LABELS[filter].toLowerCase()}${total !== 1 ? "s" : ""}` : ""}
          </p>
        </div>
        {filter === "PENDING" && total > 0 && (
          <span className="ml-auto px-3 py-1 rounded-full bg-amber-100 text-amber-800 text-sm font-semibold border border-amber-200">
            {total} en attente
          </span>
        )}
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-gray-400">Aucun marché trouvé</div>
      ) : (
        <>
                ...
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    {market.submittedBy ? (
                      ...
                      <p className="text-xs text-gray-400 mt-1">Ajouté par un admin</p>
                    )}
                  ...
                  {market.status === "PENDING" && (
                    <div className="flex gap-2 shrink-0">
                      ...
                      <Button
                        ...
                        onClick={() => { setRejectTarget(market.id); setRejectReason(""); }}
                        disabled={actionLoading}
                      >
          ...
          <p className="text-center text-xs text-gray-400 mt-4">
            {markets.length} / {total} marché{total !== 1 ? "s" : ""} affiché{markets.length !== 1 ? "s" : ""}
          </p>
        </>
      )}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-md">
            ...
            <p className="text-sm text-gray-500 mb-4">
              Vous pouvez indiquer une raison qui sera visible par le commerçant.
            </p>
            <textarea
              value={rejectReason}
              ...
              rows={3}
              className="w-full rounded-lg border border-gray-200 p-3 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-red-300 mb-4"
            />
